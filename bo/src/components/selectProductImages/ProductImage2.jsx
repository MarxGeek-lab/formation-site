...
const ProductImage2 = ({ title, subtitle, setSelectedFiles, selectedFiles, selectedFiles2, setSelectedFiles2 }) => {
  // States
    ...
    <ListItem key={file.name} className='pis-4 plb-3'>
      <div className='file-details flex gap-4s'>
        <div className='file-preview'>
          <img width={50} height={50} alt={file.name} src={API_URL_ROOT+file} />
        </div>
        <div>
          <Typography className='file-name font-medium' color='text.primary'>
            {file}
  ...
  return (
    <div>
      ...
      <div {...getRootProps()} className='mt-3'>
        <input {...getInputProps()} />
        <Button variant='contained' color='secondary' component='span' startIcon={<i className='tabler-upload' />}>
          Sélectionner une image claire du visage
        </Button>
      </div>
      ...
      {selectedFiles2.length > 0 && (
        <>
          <Typography>Images disponible</Typography>
          <List>{fileList2}</List>
        </>
      )}
    </div>
      ...
      />
          <CardContent>
            <Typography variant="body2" color="text.secondary">
              Veuillez télécharger des images de haute qualité pour une meilleure présentation.
        ...
        <CardContent>
          <div {...getRootProps({ className: 'dropzone' })}>
            ...
            <div className='flex items-center flex-col gap-2 text-center'>
              ...
              <Typography variant='h4'>Glissez-déposez votre image ici.</Typography>
