...
const productStatus = {
  pending: { title: 'Scheduled', color: 'warning' },
  Published: { title: 'Publish', color: 'success' },
  Inactive: { title: 'Inactive', color: 'error' }
}
    ...
    () => [
     /*  {
      ...
      columnHelper.accessor('productName', {
        ...
        cell: ({ row }) => (
          <div className='flex items-center gap-4'>
            <img src={row.original.mainPhotos} width={60} height={60} className='rounded bg-actionHover' />
            <div className='flex flex-col'>
              <Typography className='font-medium' color='text.primary'>
                {row.original?.name}
              </Typography>
              <Typography variant='body2'>{row.original?.location?.city+", "+row.original?.location?.district}</Typography>
              <Typography variant='body2'>PR{row.original?._id}</Typography>
            </div>
          </div>
        )
      }),
      columnHelper.accessor('category', {
        ...
        cell: ({ row }) => (
          <div className='flex items-center gap-4'>
          {/*   <CustomAvatar skin='light' color={''} size={30}>
      ...
      columnHelper.accessor('stateProperty', {
        header: 'Etat',
        cell: ({ row }) => <Typography>
          {statesProperty[row.original?.stateProperty]}
        </Typography>
      }),
     ...
     /*  columnHelper.accessor('stock', {
      ...
      columnHelper.accessor('status', {
        ...
        cell: ({ row }) => (
          <Chip
            label={statusProperty[row.original.statusProperty]}
            variant='tonal'
            color={'info'}
            size='small'
          /> 
      ...
      columnHelper.accessor('statuss', {
        ...
        cell: ({ row }) => (
        <Chip
            label={row.original.statusValidate}
            variant='tonal'
            color={row.original.statusValidate === 'en cours' ? 'warning':row.original.statusValidate === 'rejetée' ? 'error':'success'}
            size='small'
          /> 
        ...
        cell: ({ row }) => (
          <div className='flex items-center'>
            <OptionMenu
              ...
              options={[
                { text: 'Modifier', icon: 'tabler-edit' },
                {
                  text: 'Supprimer',
                  icon: 'tabler-trash',
                  menuItemProps: { onClick: () => setData(data?.filter(item => item._id !== row.original?.id)) }
                },
