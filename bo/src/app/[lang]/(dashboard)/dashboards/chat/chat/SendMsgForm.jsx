...
const SendMsgForm = ({ dispatch, allChatStore, setMessages,  activeUser, chatId, updateChatStore, isBelowSmScreen, messageInputRef }) => {
  // States
  ...
  const handleSendMsg = async (event, msg) => {
    ...
    if (msg.trim() !== '') {
      ...
      try {
        ...
        const { data, status } = await addMessage(chatId, formData);
        hideLoader();

        if (status === 200) {
          showToast('Message envoyé !', 'success', 500)
          setMsg('');
          
          const chatIndex = allChatStore.findIndex(chat => chat._id === chatId);
          if (chatIndex !== -1) {
            allChatStore[chatIndex] = data;
            setMessages(allChatStore);
          }
          ...
          showToast('Message non envoyé. Veuillez réessayer', 'error', 500)
        }
      } catch (error) {
        console.log(error);
      }
