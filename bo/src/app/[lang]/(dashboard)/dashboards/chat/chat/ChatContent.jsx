...
const UserAvatar = ({ activeUser, setUserProfileLeftOpen, setBackdropOpen }) => (
  ...
  >
    ...
    <div>
      ...
      <Typography variant='body2'>{activeUser?.role === 'owner' ? 'Propriétaire':activeUser?.role === 'agent'?'Agent démarcheur':'Client'}</Typography>
    </div>
  </div>
)
...
const ChatContent = props => {
  ...
  return chatStore.length === 0 ? (
    <CardContent className='flex flex-col flex-auto items-center justify-center bs-full gap-[18px] bg-backgroundChat'>
      ...
      <Typography className='text-center'>Sélectionnez un contact pour entamer une conversation.</Typography>
      {isBelowMdScreen && (
        <Button
          variant='contained'
