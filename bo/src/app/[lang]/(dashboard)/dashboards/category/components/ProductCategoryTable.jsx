...
const ProductCategoryTable = ({ fetchCategories, categories }) => {
  // States
  const { deleteCategory, updateStatusCategory } = useAdminStore();
  ...
  const handleDelete = (data) => {
    setMsg('Voulez-vous vraiment supprimer cette catégorie');
    setAction(1);
    setShowDialog(true);
  ...
  const handleUpdate = (data) => {
    console.log(data)
    setMsg(`Voulez-vous vraiment ${data?.isActive ? 'désactivé':'activé'} cette catégorie`);
    setAction(2);
    setShowDialog(true);
    setCategoryData(data);
  }
  ...
  const handleAction = async () => {
    ...
    try {
      let res;
      if (action === 1) {
        res = await deleteCategory(categoryData?._id);
      } else if (action === 2) {
        res = await updateStatusCategory(categoryData?._id, {isActive: !categoryData?.isActive})
      }
      hideLoader();
      ...
      if (res === 200) {
        ...
        if (action === 1) {
          showToast('Catégorie supprimé !', 'success', 5000);
        } else if (action === 2) {
          showToast(`Catégorie ${categoryData?.isActive ? 'désactivé':'activé'} avec succès.`, 'success', 5000);
        }
      } else {
  ...
  return (
    <>
      <Typography variant='h5' className='mb-6'>Liste des catégories</Typography>
       {showDialog && (
        <ConfirmationDialog
          title="Confirmation"
          message={msg}
                    ...
                    return (
                      <tr key={row.id} className={classnames({ selected: row.getIsSelected() })}>
                        {row.getVisibleCells().map(cell => (
                          <td key={cell.id} style={{ padding: '10px' }}>{flexRender(cell.column.columnDef.cell, cell.getContext())}</td>
                        ))}
                      </tr>
                    )
                  })}
