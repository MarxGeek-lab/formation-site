...
const DetailHeader = ({ withdrawalData, order }) => {
    ...
    <div className='flex flex-wrap justify-between sm:items-center max-sm:flex-col gap-y-4'>
      <div className='flex flex-col items-start gap-1'>
        <div className='flex items-center flex-wrap gap-2'>
          <div>
            <Typography variant='h5'>{`Demande #PAY-${order}`}</Typography>
      {/*       <Chip
              variant='tonal'
              label={withdrawalData && ` ${status[withdrawalData?.status]?.title}`}
              color={status[withdrawalData?.status]?.color}
              size='small'
            /> */}
          </div>
        </div>
