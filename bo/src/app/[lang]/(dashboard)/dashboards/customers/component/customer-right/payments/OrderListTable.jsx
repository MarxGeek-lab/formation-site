  ...
  const columns = useMemo(
    () => [
        ...
        cell: ({ row }) => {
          return (
            <Typography variant='body2'>
              PAY-{row.original?._id.toString().slice(0, 6).toUpperCase()}
            </Typography>
          );
        }
      }),
      columnHelper.accessor('productName', {
        ...
        cell: ({ row }) => (
          <div className='flex items-center gap-4'>
            <Typography variant='body2'>
              ORD-{row.original?.order?.slice(0, 6).toUpperCase()}
            </Typography>
          </div>
        )
      }),
      ...
      columnHelper.accessor('status', {
        ...
        cell: ({ row }) => (
          <Chip
              label={statusPayObj[row.original?.status]?.text}
              variant='tonal'
              color={statusPayObj[row.original?.status]?.color}
              size='small'
            />
        )
      }),
      columnHelper.accessor('action', {
        ...
        cell: ({ row }) => (
          <div className='flex items-center'>
            <Link href={`/${locale}/dashboards/payments/details/${row.original._id}`}>
              <Button size='small'>
  ...
  return (
    <Card>
      <CardContent className='flex justify-between flex-col items-start sm:flex-row sm:items-center gap-y-4'>
        <Typography variant='h5'>Paiements</Typography>
        {/* <DebouncedInput
          value={globalFilter ?? ''}
