...
import { useAffiliationStore } from '@/contexts/AffiliationContext';
...
export default function AffiliationPage({ params }: { params: { locale: string } }) {
  ...
  const { subscribeAffiliate } = useAffiliationStore();
  ...
  const handleAffiliate = async () => {
    if (user) {
      showLoader();
      try {
        const res = await subscribeAffiliate(user?._id);
        hideLoader()
        console.log(res);
    ...
    sx={{pb: 12}}>
      ...
      {isAffiliate && (
        ...
        <Box sx={{
          ...
          <Typography variant='h4'>Félicitation !</Typography>
          <Typography
            sx={{
              mt: 2,
              textAlign: 'center'
            }}>
            Vous avez rejoint notre programme d'affiliation !
            Dans votre tableau de bord, vous trouverez votre URL de 
            référence et des informations détaillées pour vérifier les commissions, les visites, les gains et les paiements.
          </Typography>
          <Button color='primary'
            ...
            Aller dans mon tableau de board
          </Button>
        </Box>
        </Container>
      )}

      {isAffiliate || token ? null : (
