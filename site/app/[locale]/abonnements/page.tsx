...
const OrdersPage = ({ params }: { params: { locale: string } }) => {
  ...
  const { ordersSubscription, getUserOrdersSubscription } = useOrderStore()
  ...
  const fetchData = async () => {
    if (user) {
      try {
        const { data, status } = await getUserOrdersSubscription(user._id);
        if (status === 200) {
          setLoading(false);
        }
      } catch (error) {
    ...
    return (
      <ProtectedRoute>
        <Container maxWidth="lg" sx={{ padding: '160px 0', textAlign: 'center' }}>
          <CircularProgress />
          <Typography variant="h6">Chargement de vos abonnements...</Typography>
        </Container>
      </ProtectedRoute>
    );
  }
...
console.log(ordersSubscription)
  return (
    <ProtectedRoute>
    <Container 
      maxWidth="lg" 
    ...
    >
      ...
      <Box sx={{ mb: { xs: 3, sm: 4 } }}>
        ...
        >
          {ordersSubscription?.length} abonnement(s) trouvée(s). Consultez l'historique et suivez leur statut.
        </Typography>
      </Box>

      {ordersSubscription.length === 0 ? (
        <Paper sx={{ 
          p: { xs: 3, sm: 4 }, 
          textAlign: 'center',
                ...
                >
                  ...
                  >
                    <Box>
                      ...
                      >
                        Abonnement ORD-{order?._id?.toUpperCase()}
                      </Typography>
                  ...
                  {order?.items?.map((item: any, idx: number) => (
                    ...
                    >
                      ...
                      <Box sx={{ flexGrow: 1 }}>
                        ...
                        <Typography variant="body2" color="text.secondary">
                          Durée: {item?.subscription?.duration} mois
                        </Typography>
                        <Typography variant="body2" color="text.secondary">
                          Expire le: <Chip 
                          icon={<CalendarMonth />}
                          label={dayjs(item?.subscriptionExpiredAt).format('DD/MM/YYYY HH:mm:ss')}
                          size="medium"
                          color="primary"
                          />
                        </Typography>
                        <Typography variant="body2" color="text.secondary">
                          Produits: {item?.subscription?.relatedProducts?.length}
                        </Typography>
                        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                          {item?.subscription?.relatedProducts?.map((product: any) => (
                            <Typography
                              key={product._id}
                              variant="body2"
                              color="text.secondary"
